...
class Ur5Moveit:
    ...
    def sheetpush_dispatch(argv,arg_orderid,city,arg_item,arg_prior,arg_quantity,arg_cost,arg_status,disp_date_time):
        ...
        ur5.send_goal("http","pub","NA",str_payload1)
    ...
    def sheetpush_incoming(argv,arg_orderid,arg_time,arg_item,arg_prior,arg_quantity,city,lon,lat,arg_cost):
        ...
        ur5.send_goal("http","pub","NA",str_payload)
    """
    ...
    #    try:
        ...
        try :
            ...
            if (queue_item==1):
                ...
                ur5.high_priority()
                #ur5.sheetpush_dispatch(self.order_id,self.city,self.x,self.prior,self.qty,self._cost,"Dispatched","aa")
                #ur5.sheetpush_dispatch(ur5.orderid_queue.pop(0),ur5.city_queue.pop(0),"Medicines","HP","1","450","Dispatched",ur5.get_time_str())
                ur5.sheetpush_dispatch(ur5.orderid_queue.pop(0),ur5.city_queue.pop(0),"","","","","Dispatched","")
            if (queue_item==2):
                print("got it 2") 
                ur5.hard_set_joint_angles(lst_joint_angles_belt, 5)
                ur5.mid_priority()  
                
                #ur5.sheetpush_dispatch(ur5.orderid_queue.pop(0),ur5.city_queue.pop(0),"Food","MP","1","250","Dispatched",ur5.get_time_str())
                ur5.sheetpush_dispatch(ur5.orderid_queue.pop(0),ur5.city_queue.pop(0),"","","","","Dispatched","")
            if (queue_item==3):
                print("got it 3") 
                ur5.hard_set_joint_angles(lst_joint_angles_belt, 5)
                ur5.low_priority()
                #ur5.sheetpush_dispatch(ur5.orderid_queue.pop(0),ur5.city_queue.pop(0),"Clothes","LP","1","100","Dispatched",ur5.get_time_str()) 
            #if a==0 :    
            #    ur5.sheetpush_dispatch("1","","","","","","Dispatched","aa")
            #    a=2
            #if b==0 :    
                p=ur5.get_time_str(5)
                rospy.logwarn(p)
                ur5.sheetpush_dispatch(ur5.orderid_queue.pop(0),ur5.city_queue.pop(0),"Clothes","LP","1","100","Dispatched",str(p))
                rospy.sleep(2)           
            #    b=2
            #if h==0 :    
